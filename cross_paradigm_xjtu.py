...
from config import GBSA_BASE, GBSA_AUG, LG_PARAMS, ROLLING_WINDOW_XJTU, EVAL_TIME_QUANTILES, N_EVAL_TIMES
...
def main():
    ...
    ap.add_argument('--out', default='results_xjtu.json')
    ap.add_argument('--optimize', action='store_true', help='re-optimise LG via Optuna instead of using config.LG_PARAMS')
    args = ap.parse_args()
    data = xjtu_data.load(args.data)
    if data is None:
        print('ERROR: XJTU-SY data not found. Pass --data PATH.')
        return
    print('=' * 78)
    print('XJTU-SY BEARINGS — SURVIVAL CROSS-PARADIGM')
